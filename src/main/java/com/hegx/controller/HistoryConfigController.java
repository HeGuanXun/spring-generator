...
import com.hegx.models.HistoryConfigCVF;
...
public class HistoryConfigController extends BaseController {
	...
	public void initTable() {
		...
		tdInfo.textProperty().bind(SpringGeneratorApplication.LANGUAGE.get(LanguageKey.CONFIG_TD_INFO));
		TableColumn<HistoryConfigCVF, String> tdOperation = new TableColumn<HistoryConfigCVF, String>("操作");
		tdOperation.textProperty().bind(SpringGeneratorApplication.LANGUAGE.get(LanguageKey.CONFIG_TD_OPERATION));
		...
		StringProperty property = SpringGeneratorApplication.LANGUAGE.get(LanguageKey.HISTORY_CONFIG_TABLE_TIPS);
		...
		lblTips.textProperty().bind(SpringGeneratorApplication.LANGUAGE.get(LanguageKey.CONFIG_LBL_TIPS));
		LOG.debug("初始化配置信息完成!");
	...
	public ObservableList<HistoryConfigCVF> getHistoryConfig() throws Exception {
		...
		for (HistoryConfig tmp : item) {
			...
			button.textProperty().bind(SpringGeneratorApplication.LANGUAGE.get(LanguageKey.CONFIG_BTN_LOAD));
			...
			button1.textProperty().bind(SpringGeneratorApplication.LANGUAGE.get(LanguageKey.CONFIG_BTN_DATELE));
