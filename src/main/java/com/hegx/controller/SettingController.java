...
public class SettingController extends BaseController {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		lblLanguage.textProperty().bind(SpringGeneratorApplication.LANGUAGE.get(LanguageKey.SETTING_LBL_LANGUAGE));
		...
		lblEnglish.setOnMouseClicked(ev -> {
			SpringGeneratorApplication.loadLanguage(Locale.ENGLISH);
			closeDialogStage();
		});
		lblSimpleChinese.setOnMouseClicked(ev -> {
			SpringGeneratorApplication.loadLanguage(Locale.SIMPLIFIED_CHINESE);
			closeDialogStage();
		});
	}
